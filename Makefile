...
# LC quartz is a large cluster of Intel Haswell nodes
  ...
  computername := quartz
   ...
   FC  = mpif90
   CXX = nvcc
   RAJA_LOCATION = /g/g90/choi18/raja-v0.7.0/install
   CUDA_LIBS = $(CUDA_HOME)/lib64
   OPT = -DUNRAJA=1 -O3 -dc -ptx -Xptxas -dlcm=cg -ccbin mpicxx -Xcompiler="" -std=c++11 --expt-extended-lambda -restrict -arch=sm_70 -DRAJA03=1 -I$(RAJA_LOCATION)/include  --x cu -DUSE_NVTX -DRAJA_USE_CUDA -DSW4_CROUTINES -DRAJA_USE_RESTRICT_PTR -DCUDA_CODE -DRAJA_ENABLE_NESTED
